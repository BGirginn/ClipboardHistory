...
CoreDeck handles sensitive local data. Please do not disclose a suspected vulnerability in a public issue. Use GitHub's private vulnerability-reporting form for this repository. If that form is unavailable, contact the repository owner through the private contact method on the GitHub profile and include `CoreDeck security` in the subject.
...
Only versions listed as supported in the latest release receive fixes. The currently supported prerelease is `v1.0.0-beta.6`; earlier beta builds should be upgraded before reporting a defect.
