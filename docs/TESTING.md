...
| Coverage | Every executable production Swift line, per file and aggregate, 100% | Last pre-signing-change result was 73.14% (8,360/11,430); current-source merge must be regenerated; blocked |
| UI automation | Status item, shortcut, panel/menu tracking, keyboard, settings, lock, paste, drag/drop, import/export | 6 isolated ad-hoc-signed UI tests passed; protected self-signed full-flow coverage remains pending |
...
| Distribution | Stable self-signed certificate and encrypted backup, quarantined clean-user install, checksums, SPDX SBOM, Cask audit/install/upgrade/uninstall | Stable Community identity and `syft` 1.50.0 arm64 are installed; encrypted `.p12` backup, clean-machine evidence, tag, Release, and Cask are absent |
...
The self-signed UI run is a separate local gate. It requires the stable Community identity but no Apple account or provisioning profile:
