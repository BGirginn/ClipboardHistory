...
The Control Center is now a compact Quick Center with recent Clipboard actions and a Quick Note editor backed by the same draft and revision state as Notes. Settings opens in one reusable sidebar window and remembers its last section. CoreDeck's own menu-bar modules can be placed in a drawer and restored without requesting Accessibility permission.
...
External system and third-party menu-bar items are not managed in beta.6. The production domain and crash-safe journal are present, but the external movement adapter remains disabled until occupied-space reclamation and restore behavior are accepted on the supported macOS matrix.
