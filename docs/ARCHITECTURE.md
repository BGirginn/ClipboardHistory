...
`ExportImportService` authenticates password archives, validates counts, sizes, managed paths, and SHA-256 manifests, materializes into isolated staging storage, and verifies the result. `CommunityMigrationService` closes SQLite before filesystem moves, preserves the existing destination as a rollback backup, and only swaps the verified staging directory into place.

Two signing/key modes exist. Apple Development uses Data Protection Keychain. `CommunityRelease` uses the login Keychain, removes unauthorized keychain-access-group entitlements, and is intended for one stable, offline-created self-signed identity. The certificate's private key must never enter Git. The community artifact is not Apple-notarized and must not bypass Gatekeeper or quarantine.
