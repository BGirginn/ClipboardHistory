...
CoreDeck is a local-first modular menu-bar application. `AppModel` is the composition root for shared services and independently owns Clipboard History, Notes, Keyboard Cleaning, Scroll Reverse, System Monitor, Audio Mixer, Control Center, and Settings models. `FeatureRegistry` is the compile-time catalog; `MenuBarConfigurationStore` persists versioned placement and validated click actions in `UserDefaults`. Configuration v5 migrates legacy standalone booleans to `Hidden` or `Always` and adds supported `When Active` policies and ordered metric settings. Configuration v6 migrates visible metrics to independently movable status items while preserving their order and formats. Combined and separate layouts share one snapshot producer and preserve existing `NSStatusItem` identities. CPU and RAM use compact stacked labels, while temperature is value-only; changing values mutates existing AppKit labels instead of rebuilding status items. `AppRouter` owns the current `AppFeature`, selected Settings section, and Settings return point. `ClipboardHistoryAppDelegate` keeps the process in accessory mode while the main CoreDeck status item is enabled; hiding that item switches to regular activation policy so Finder and Dock reopen events can present `AppShellView` through `ApplicationWindowController` in a standard window. `MenuBarController` dynamically owns the enabled module/metric status items, one shared feature popover, a reusable own-module drawer popover, detachable panel, Quick Look, and outside-click behavior, and falls back to the application window when a shortcut has no status-item anchor. `PanelCloseCoordinator` defers panel closure during context-menu and modal confirmation tracking so a command can complete without leaving the UI stuck.

Configuration v7 adds own-module drawer placement and its separate status item. Moving a module into the drawer retains its prior `Hidden`/`When Active`/`Always` policy; disabling drawer placement restores that policy, while the explicit restore action makes a previously hidden module visible. System Monitor placement suppresses the top-bar metric demand while preserving metric order, formats and combined/separate preference. Existing v6 combined layouts survive migration. Both popovers use the same feature controllers and are mutually exclusive; drawer content is created once and follows application appearance. External item discovery, space reclamation, native activation and recovery remain experimental and are not part of the production drawer.

`ClipboardHistoryViewModel` is now Clipboard-only. Its mutation, privacy/settings operations, capture, presentation, interaction/write-coordination, archive, and monitor-delegate facets live in separate files and retain the façade's main-actor isolation. `NoteController` owns note search, the mutually exclusive `NoteScreen.list`/`.editor` state, validation, and revision-aware serialized debounced/forced saves. `InputToolsController` owns the shared event-tap coordinator plus `KeyboardCleaningController` and `ScrollReversalController`; workspace lifecycle handling no longer leaks into the Clipboard model. `SettingsFeatureModel` composes app-wide Settings, while each feature-specific Settings view observes its actual controller. Generic Settings opens a grouped, labeled index without preselecting a pane; feature entry points can still route directly to their owning settings subsection. Sensitive Clipboard previews and restore operations authenticate through the injected `SystemAuthenticating` boundary.

`InputEventTapCoordinator` owns one dynamic session event tap shared by Keyboard Cleaning and Scroll Reverse. Keyboard Cleaning discards keyboard/media events until the user stops it while leaving mouse/scroll available; lifecycle and event-tap failure paths always release the keyboard. Scroll Reverse mutates enabled line-based or precise axes in place. Neither path records or logs input content. Direct paste and Input Tools ask macOS for Accessibility access. Timer, authentication, Accessibility trust/event creation, launch-at-login, pasteboard, drag provider, Quick Look presentation, status-item/panel creation, content analysis, storage-operation failure, filesystem migration, and master-key boundaries are injectable.
...
`ClipboardMonitor` observes `changeCount` on the common AppKit run-loop modes, renews its delivery baseline after pasteboard-server counter resets, records pasteboard identity, ignores known transient/concealed/auto-generated types, and optionally ignores Universal Clipboard or custom UTIs. Transient, unsupported, excluded, paused, and application-generated changes never reach Clipboard insertion. Deletion clears the system pasteboard only while the stored identity still matches, preserving clipboard data copied after the history item was recorded. A cancelled quit resumes capture only when the monitor had been active before shutdown began.
...
Browser audio control crosses process boundaries through the bundled `ClipboardHistoryBrowserAudioBridge.xpc` executable registered as an on-demand `SMAppService` LaunchAgent with one advertised Mach service. The main app exports the controller callback; the Chromium native host and Safari extension exchange only versioned bounded data messages. The service validates the live caller signature, allowlisted bundle identifier, UID, and signing-certificate equality before accepting a connection.
...
The main target builds `CoreDeck.app` with a `CoreDeck` executable while retaining the `ClipboardHistory` Xcode target, scheme, and Swift module name. Debug, Release, and `CommunityRelease` use an empty entitlement file and the stable self-signed `ClipboardHistory Community Beta` identity. The main bundle identifier, helper and service identifiers, Application Support directory, UserDefaults keys, and Keychain service remain unchanged for upgrade compatibility. The classic login Keychain protects Notes and supports one-time migration of legacy encrypted Clipboard records; current Clipboard storage is open and has no startup Keychain dependency. The embedded `ClipboardHistoryLoginItem` helper launches the main app with `--background-launch` and exits, keeping login launch silent; the launch-at-login service migrates the former `SMAppService.mainApp` registration to this helper. No Apple account or provisioning profile is required. The certificate's private key must never enter Git. The community artifact is not Apple-notarized and must not bypass Gatekeeper or quarantine.
...
│   │   ├── Drawer/           # Own-module shelf using shared feature controllers
